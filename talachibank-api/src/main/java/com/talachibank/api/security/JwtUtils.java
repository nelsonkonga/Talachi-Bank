...
import com.talachibank.api.service.TokenBlacklistService;
...
public class JwtUtils {
    ...
    @Value("${talachibank.app.jwtSecret}")
    private String jwtSecret;

    @Value("${talachibank.app.jwtExpirationMs}")
    private int jwtExpirationMs;

    @Value("${talachibank.app.jwtRefreshExpirationMs}")
    ...
    public boolean validateJwtToken(String authToken) {
        try {
            ...
            if (authToken.chars().filter(ch -> ch == '.').count() >= 3) {
